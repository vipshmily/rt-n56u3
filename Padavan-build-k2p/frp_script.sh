...
if [ ! -f "/tmp/frpc" ] ; then
	curl -L -k -s -o /tmp/frpc --connect-timeout 10 --retry 3 https://cdn.jsdelivr.net/gh/eprea/cdn/frpc && chmod -R 777 /tmp/frpc
else
...
if [ ! -f "/tmp/frps" ] ; then
	curl -L -k -s -o /tmp/frps --connect-timeout 10 --retry 3 https://cdn.jsdelivr.net/gh/eprea/cdn/frps && chmod -R 777 /tmp/frps
else
